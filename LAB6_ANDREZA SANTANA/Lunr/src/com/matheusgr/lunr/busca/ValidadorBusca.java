...
class ValidadorBusca extends ValidadorPadrao {
	...
	/**
	 * Valiados os metadados da busca avançada. A chave e o metadado não podem ser vazios.
	 * 
	 * @param metadados O mapa de metadados que serão usados na busca.
	 */
	public void valida(Map<String, String> metadados) {
		...
		for (String chave : metadados.keySet()) {
			if (chave.isBlank() || metadados.get(chave).isBlank()) {
				throw new IllegalArgumentException("Os metadados não devem ser vazios");
			}
		}
	}
